...
def extract_spec(spec_div: bs4.Tag) -> (str, str):
    value = spec_div.find('span', {"class":"e1ckvoeh0 e106ikdt0 app-catalog-1uhv1s4 e1gjr6xo0"}).span.text
    key = spec_div.find('span', {"class":"e1ckvoeh3 e106ikdt0 app-catalog-1eqtzki e1gjr6xo0"}).span.text
    return (key, value)
...
def parse_main(block: bs4.Tag) -> dict[str: any]:
    ...
    if "Частота графического" in raw_specs.keys():
...
def parse_memory(block: bs4.Tag) -> dict[str: str]:
    ...
    if "Объем" in raw_specs.keys():
        specs["VRAM"] = int(NUMBER_PATTERN.findall(raw_specs['Объем'])[0])
    else:
        specs["VRAM"] = "NULL"
    
    if "Пропускная способность памяти" in raw_specs.keys():
...
def parse_sockets(block: bs4.Tag) -> dict[str: str]:
    ...
    if "Разъемов" in raw_specs.keys():
        specs["HDMI_count"] = int(NUMBER_PATTERN.findall(raw_specs["Разъемов"])[0])
    else:
        specs["HDMI_count"] = "NULL"
    if "Количество разъемов" in raw_specs.keys():
        specs["DisplayPort_count"] = int(NUMBER_PATTERN.findall(raw_specs["Количество разъемов"])[0])
    else:
